...
- **10 data layers**: flights, satellites, disasters, asteroids, weather, cameras, livestreams, news, militaryActions, carparks
...
## Singapore Carparks Layer (Hackathon Addition)
- `/api/carparks` — LTA DataMall `CarParkAvailabilityv2` with `AccountKey` header
- `lib/api/carparks.ts` — Fetcher with pagination ($skip), Location string parsing ("lat lon")
- `hooks/useCarparks.ts` — 60s polling interval
- `components/layers/CarParkLayer.tsx` — Point entities (green/amber/red by availability) + 500m ellipses (heatmap)
- Thresholds: >100 lots = green, 20-100 = amber, <20 = red
- `distanceDisplayCondition`: points visible below 500km, labels below 50km, ellipses below 300km
- Requires `LTA_DATAMALL_KEY` in `.env.local` (free from datamall.lta.gov.sg)
